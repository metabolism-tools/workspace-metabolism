...
> **2026-09-08 复核更正**：下文记录的是早期运行过程，不能作为当前自动删除字段的依据。后续真实消费者检查发现，IC/PIT 证据仍可能被下游读取；“旧纪元关闭后无人再读”未获完整证据支持。当前生产已升级 wm 0.5.2、保护必要字段，只运行观察入口，**自动删字段保持关闭**。历史回收量不等于下游正确性或长期无人值守已获证明。最新验收见文末补充。
...
### 7.6 无人值守兜底：让心跳自己补跑 slim

前两轮都靠"人发现 → 人修复"闭环。owner 明确要求**不再人工干预**后，
我们给心跳加了 `--auto-slim`：维护窗口（引擎停机后）的 06:15 心跳在
DB ≥ 临界阈值时自动执行 `wm slim`。六个守卫防止误触发：

1. **DB ≥ 15 GB 临界**才考虑（低于临界零开销跳过）；
2. **引擎确已停止**：无 tick 进程 + 对 marathon.lock 做非阻塞 flock
   探测——不能只看锁文件是否存在，因为引擎停止后锁文件残留（实测
   lock_age 157 分钟而零进程）；
3. **24 h 冷却**：上次成功 slim 后不重复（防每 15 分钟反复触发）；
4. **flock 非阻塞**：与 05:50 定时 slim 永不并发；
5. 失败 → 心跳追加 `auto_slim_failed` 临界告警推飞书；
6. 结果进心跳 payload + 状态文件，哈希链审计照常。

实测（真实 DB、真实 wm、线上代码）：把阈值临时降到当前库大小之下，
触发路径 2.5 秒跑完 `wm slim`（扫描 417,623 行、幂等零剥离）、
冷却拦截生效、状态文件正确落盘。之后日常形态：**05:50 定时主链路 +
06:15 心跳兜底 + 心跳告警安全网**，三层全自动，无人值守。
...
| D3 | 无人值守兜底上线并试跑 | 心跳 `--auto-slim`（06:15 维护窗口）；真实 DB 试跑 2.5 s 完成、冷却拦截、状态落盘；主链路 + 兜底 + 告警三层全自动 |
...
## 10. 后续反例与受保护上线（2026-09-08）

这次维护的[可运行评价与下一轮记录方法](maintenance-cycle-runbook.md)已补充：以完整维护机会计量，不以回收字节或文件数量作为自主能力指标。当前真实输入仍缺下一段研究工作流回执和完整人工记录，因此不认定低监督收益已经成立。

后续检查推翻了早期“重字段不再需要”的笼统判断：真实消费者会从逐期 IC 等证据构造研究输入。隔离实验中故意删除这些字段，消费者输出随之改变；因此旧回收量不能沿用为安全收益，也不能据此断言历史所有下游结果已经损坏。

wm 0.5.2 补充关系键保留与必要字段保护，生产已安装并合并受保护规则。七组合成实验覆盖错误关系、近期保留、字段保护及破坏性负对照；可执行不代表可自动放行，生产自动删除字段仍关闭。观察任务已上线，手动运行同入口成功；定时执行与长期稳定性需要后续实际证据。

约 13.8 GB 真实数据库的一致性备份与恢复，在云端独立副本通过整库哈希和 SQLite 检查。完整预览扫描 468,134 行，按受保护规则预计改动 0 行、回收 0 字节；因此没有为制造“维护成功”而执行删字段。约 27.6 GB 临时副本已回收。首次演练因最终回执未返回而结果未知，随后增加逐阶段持久回执，不能把首次运行补记成功。

空间治理转向业务系统已有的无损共享编码工具：最终279份矩阵全部完成迁移，替换前逐字节还原验证通过，并完成分批额外读取复核。最终只读检查全部279份引用和60个共享压缩块的内容哈希通过；目录占用从67,849,958,357降至1,416,146,145字节，扣除预检和执行新增共享块后净减少66,433,812,212字节（约66.43 GB），与逐批回执一致。共享编码是业务系统工具的成果，不能冒充wm新增功能；wm在这里提供维护边界、保护规则和运行证据组织。279份文件并非279个独立事故或独立研究样本。

本轮尚未计量人工监督分钟、Token、净总成本或下游长期错误率。可检验的研究问题仍是：**哪些证据与恢复条件足以允许某一类维护自主完成，同时让下游判断保持正确？** 无损重编码是已有方法；本案例贡献在于可追溯反例、受保护执行和拒绝无收益动作，不宣称方法首创。
