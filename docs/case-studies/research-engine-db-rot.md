...
| D3 | 手动首跑成功 | **21.7 GB → 11.3 GB，回收 10.15 GB**；扫描 417,623 行、剥离 299,683 行；VACUUM 完成；DB 关键告警降级为预警 |
| D3 | 效果检测又发现第三个真问题 | 死单元学习窗口被"干净纪元"稀释 → 27 个整批全灭 batch；改为全历史学习（sticky）后 807 死单元不再复活 |
